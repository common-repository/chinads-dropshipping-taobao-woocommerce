...
class Download_Images extends \WP_Background_Process {
    protected static $instance = null;
    /**
     * @var string
     */
    protected $action = 'tbds_background_download_images';
    ...
    protected function task( $item ) {
        ...
        try {
            ...
            if ( $post && $post->post_type === 'product' && $src ) {
                $thumb_id = Utils::download_image( $image_id, $src, $product_id );
                if ( $thumb_id && ! is_wp_error( $thumb_id ) ) {
                        ...
                        if ( count( $product_ids ) ) {
                            foreach ( $product_ids as $v_id ) {
                                ...
                                if ( $post_type === 'product' ) {
                                    ...
                                    if ( $parent_id ) {
										Taobao_Post::update_post_meta( $parent_id, '_tbds_product_image', $thumb_id );
                                    }
                    ...
                    Error_Images_Query::insert( $product_id, implode( ',', $product_ids ), $src, intval( $set_gallery ) );
                    error_log( TBDS_CONST['basename'] . ': ' . $thumb_id->get_error_code() . ' - ' . $thumb_id->get_error_message() );
                }
            }

        } catch ( \Exception $e ) {
            Error_Images_Query::insert( $product_id, implode( ',', $product_ids ), $src, intval( $set_gallery ) );
            error_log( TBDS_CONST['basename'] . ': ' . $e->getMessage() );
